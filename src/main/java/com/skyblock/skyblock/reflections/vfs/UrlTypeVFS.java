...
public class UrlTypeVFS implements Vfs.UrlType
{
    ...
    public URL adaptURL(final URL url) throws MalformedURLException {
        if ("vfszip".equals((Object)url.getProtocol())) {
            return this.replaceZipSeparators(url.getPath(), (Predicate<java.io.File>)(file -> file.exists() && file.isFile()));
        }
        if ("vfsfile".equals((Object)url.getProtocol())) {
            return new URL(url.toString().replace((CharSequence)"vfsfile", (CharSequence)"file"));
        }
        return url;
    }
    ...
    URL replaceZipSeparators(final String path, final Predicate<java.io.File> acceptFile) throws MalformedURLException {
        ...
        while (pos != -1) {
            ...
            if (pos > 0) {
                ...
                if (acceptFile.test((Object)file)) {
    ...
    int findFirstMatchOfDeployableExtention(final String path, final int pos) {
        ...
        final Matcher m = p.matcher((CharSequence)path);
        if (m.find(pos)) {
            return m.end();
        }
        return -1;
    ...
    URL replaceZipSeparatorStartingFrom(final String path, final int pos) throws MalformedURLException {
        ...
        for (final String ext : UrlTypeVFS.REPLACE_EXTENSION) {
            while (zipPath.contains((CharSequence)ext)) {
                zipPath = zipPath.replace((CharSequence)ext, (CharSequence)(ext.substring(0, 4) + "!"));
                ++numSubs;
            }
        }
        String prefix = "";
